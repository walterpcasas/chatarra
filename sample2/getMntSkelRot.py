...
if flag == 1:
    ...
    for number, img in enumerate(imgs):
        ...
        fileMnt = os.path.join(mntImages, prefix + '.mnt')
        filePred = os.path.join(predMnt, prefix + '-'+angle+ '.mnt')
        file = imread(os.path.join(inImages, img))


        # Minutiae Predict
        df_pred = pd.read_csv(filePred, delim_whitespace=True, header=None, skiprows=2)
        ...
        # Minutiae Input
        df = pd.read_csv(fileMnt, delim_whitespace=True, header=None, skiprows=2)

        mnt = np.zeros_like(file)

        for index, row in df.iterrows():
            mnt[int(row[1])][int(row[0])] = 1
